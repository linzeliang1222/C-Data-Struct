...
int x, y, nextx, nexty, wint, turn, now, you;
int maxm, maxy, maxmcoordx, maxmcoordy, maxycoordx, maxycoordy;
int map[H][H];
...
try_again:
	...
	/* 根据用户输入进行移动，第一个if防止光标和黑白子交换，第二个if防止当前坐标变成黑白子，跟着走 */
	switch(i)
	{
		case LEFT:
				nextx = x;
...
{
	int i, j;
	int score = 0, coord[H*H][2] = {0}, max = 0, t = 0;//t来计数
	for (i = 4; i < H-4; i++)
	{
		for (j = 4; j < H-4; j++)
		{
